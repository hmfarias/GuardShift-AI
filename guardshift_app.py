...
The message below may include one 'Objetivo', one 'Fecha', and multiple 'DNI + Nombre + Horario'.

Extract the data in table format with columns:
...
\"\"\"
        ...
        if not result_data:
            ...
            df = df[~df.apply(lambda row: all(re.fullmatch(r"[-–—\s]*", str(cell)) for cell in row), axis=1)]

            def format_time(value):
                value = value.replace("hs", "").replace("HS", "").strip()
            ...
            def format_date(value):
                try:
                    ...
                    return date_obj.strftime("%d/%m/%Y")  # Esto es redundante pero asegura el formato correcto
                except:
                    try:
                        # Intenta con otro formato común (por si viene como d/m/yyyy, sin ceros)
                        date_obj = datetime.strptime(value.strip(), "%d/%m/%y")
                        return date_obj.strftime("%d/%m/%Y")
                    except:
                        return value  # Devuelve sin cambios si no puede parsear
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix=".xlsx") as tmp_xlsx:
                df.to_excel(tmp_xlsx.name, index=False, startrow=0, startcol=0)

                st.success("✅ Archivo procesado con éxito!")
                st.dataframe(df)

                with open(tmp_xlsx.name, "rb") as f:
                    st.download_button("⬇️ Descargar Archivo Excel", data=f, file_name=filename)
